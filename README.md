...
本项目是一个**教学辅助工具**，并非专业化学信息学软件，也不构成任何形式的专业意见。

- 软件按"原样"（AS IS）提供：尽管项目经过 850 项自动化测试，中文命名解析、结构构建与各分析模块仍**可能存在未被发现的错误或边界情况**（俗名、多官能团、稠环及特殊结构尤甚），请勿将其结果视为权威结论；
- 命名遵循**教材式规则**（以人教版为主），可能与 IUPAC、其它教材版本或考试参考答案存在差异；
- 氢谱、化学位移、等效氢等均为**教学简化模拟**，不代表真实谱图数据；
- 建议教师在使用本工具进行教学、判题或考试讲解前，对**关键结果做人工核验**；
- 作者不对因使用本工具而产生的教学效果、考试成绩或其它损失承担责任（详见 [LICENSE](./LICENSE)）。

发现问题？欢迎通过 [Issues](https://github.com/Aran1Ran/chemname-visualizer/issues) 反馈——你的一句话，可能就是下一个测试用例。
